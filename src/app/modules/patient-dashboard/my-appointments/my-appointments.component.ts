...
export class MyAppointmentsComponent implements OnInit {
  ...
  surveyAppointment() {
    ...
    this.firestore
      ...
      .then(() => {
        this.toastr.success('Encuesta completada');
        this.spinnerService.hide();
      })
      .then(() => {
        if (this.specialtySelected) {
  ...
  rateAppointment() {
    if(this.appointmentRate == 0){
      this.toastr.error('Ingrese una calificacion');
    }else{
      this.spinnerService.show();
      this.firestore.rateAppointment(this.appointmentUID, this.appointmentRate).then(() => {
        this.toastr.success('Calificacion enviada');
        this.spinnerService.hide();
      }).then(() => {
        if (this.specialtySelected) {
          this.getAppointmentsBySpecialty();
  ...
  search() {
    ...
    this.allAppointments.forEach((element: { appointmentInfo: { toString: () => string; } | undefined; date: { toString: () => string; } | undefined; diagnosis: { toString: () => string; } | undefined; doctor: { toString: () => string; } | undefined; patient: { toString: () => string; } | undefined; specialty: string | undefined; status: string | undefined; observations: string | undefined; }) => {
